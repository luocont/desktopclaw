...
from desktopclaw import __logo__, __version__
...
def _init_prompt_session() -> None:
    ...
    from desktopclaw.config.paths import get_cli_history_path
...
def onboard():
    ...
    from desktopclaw.config.loader import get_config_path, load_config, save_config
    from desktopclaw.config.schema import Config
...
def _make_provider(config: Config):
    ...
    from desktopclaw.providers.base import GenerationSettings
    ...
    # Custom: direct OpenAI-compatible endpoint, bypasses LiteLLM
        ...
        from desktopclaw.providers.custom_provider import CustomProvider
    ...
    # Azure OpenAI: direct Azure OpenAI endpoint with deployment name
        ...
        from desktopclaw.providers.registry import find_by_name
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from desktopclaw.config.loader import load_config, set_config_path
...
def gateway(
    ...
    from desktopclaw.cron.types import CronJob
    from desktopclaw.heartbeat.service import HeartbeatService
    from desktopclaw.session.manager import SessionManager
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        from desktopclaw.agent.tools.message import MessageTool
        reminder_note = (
...
def channels_status():
    """Show channel status."""
    from desktopclaw.channels.registry import discover_channel_names, load_channel_class
...
def _get_bridge_dir() -> Path:
    ...
    from desktopclaw.config.paths import get_bridge_install_dir
...
def channels_login():
    ...
    from desktopclaw.config.loader import load_config
    from desktopclaw.config.paths import get_runtime_subdir
...
def status():
    """Show nanobot status."""
    from desktopclaw.config.loader import get_config_path, load_config
    ...
    if config_path.exists():
        from desktopclaw.providers.registry import PROVIDERS
